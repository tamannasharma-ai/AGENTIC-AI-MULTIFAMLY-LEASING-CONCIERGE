...
LLM_FALLBACK_CONFIG: List[Dict[str, Any]] = [
    ...
    {
        "model": os.getenv("GROQ_FALLBACK_MODEL", "groq/compound"),
        "temperature": 0.1,
        "max_retries": 2,
        "timeout": 25,
    },
...
def build_resilient_llm(tools: list, config: List[Dict[str, Any]]):
    configured_llms = [
        ChatGroq(
            model=entry["model"],
    ...
    primary_llm = configured_llms[0]
    fallback_llms = configured_llms[1:]
    if fallback_llms:
...
llm_with_tools = build_resilient_llm(tools, LLM_FALLBACK_CONFIG) if os.getenv("GROQ_API_KEY") else None

SYSTEM_PROMPT = f"""You are the 24/7 AI Leasing Concierge for {PROPERTY_NAME} located at {PROPERTY_ADDRESS}.Behave properly and humanly.Undersand the user query first and then reply as per below instructions:
...
def agent_node(state: AgentState):
    ...
    try:
        ...
        print(f"❌ Fallback exhaustion: {fatal_err}")
        emergency_reply = (
        ...
        return {"messages": [AIMessage(content=emergency_reply, response_metadata={"analytics_outcome": "model_error"})]}
